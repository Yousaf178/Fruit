...
if($_POST){
	...
	*/
	...
	$cmd  = $db->query($insert);
  if($cmd){
	  //add_customer_onlineorder.php?voucher=1&&customer=3
	   echo '<script>window.location="add_customer_onlineorder.php?customer='.$customer.'&&voucher='.$_GET['voucher'].'";</script>';
	 ...
	 <div class="col-md-3"style="border: 1px solid blue;">
		...
		<div class="col-md-2" >
           ...
           <thead>			  
			   ...
			   <tbody>
			   <?php 
			   echo "select fruit,online_order.quantity from 
				 fruits,online_order where online_order.fruit_id= fruits.id and online_voucher_id='".$_GET['voucher']."' and customer_id='".$_GET['customer']."'";
			    $list = "select fruit,online_order.quantity from 
				 fruits,online_order where online_order.fruit_id= fruits.id and online_voucher_id='".$_GET['voucher']."' and customer_id='".$_GET['customer']."'";
				 $cmd = $db->query($list);
				 $i = 1;
